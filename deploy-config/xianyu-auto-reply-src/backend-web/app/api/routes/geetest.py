...
router = APIRouter(prefix="/geetest", tags=["极验验证码"])
...
async def geetest_validate(request: GeetestValidateRequest):
    ...
    """
    try:
        logger.info(f"极验二次验证请求: challenge={request.challenge[:16]}..., validate={request.geetest_validate[:16] if request.geetest_validate else 'None'}...")
        ...
        # 判断是正常模式还是宕机模式
        # 通过存储的状态来判断：如果register时success=1则是正常模式，success=0则是宕机模式
        stored_status = geetest_status_store.get(request.challenge)
