    ...
    <div class="mx-auto max-w-6xl space-y-6">
      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
            ...
            <div class="space-y-6 p-6">
              ...
              <div>
                <label
                  class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300"
                >
                  {{ t("admin.settings.site.concurrencyLimitMessage") }}
                </label>
                <textarea
                  v-model="form.concurrency_limit_message"
                  rows="3"
                  class="input font-mono text-sm"
                  :placeholder="t('admin.settings.site.concurrencyLimitMessagePlaceholder')"
                ></textarea>
                <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">
                  {{ t("admin.settings.site.concurrencyLimitMessageHint") }}
                </p>
              </div>
        ...
        <div class="card">
          <div class="border-b border-gray-100 px-6 py-4 dark:border-dark-700">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
              {{ t('admin.settings.features.xianyuDelivery.title') }}
            </h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
              {{ t('admin.settings.features.xianyuDelivery.description') }}
            </p>
          </div>
          <div class="space-y-5 p-6">
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
                  {{ t('admin.settings.features.xianyuDelivery.enabled') }}
                </label>
                <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                  {{ t('admin.settings.features.xianyuDelivery.enabledHint') }}
                </p>
              </div>
              <Toggle v-model="form.xianyu_delivery_enabled" />
            </div>
          </div>
        </div>
...
async function saveSettings() {
  ...
  try {
    ...
    const payload: UpdateSettingsRequest = {
      ...
      concurrency_limit_message: form.concurrency_limit_message,
      ...
      // Xianyu delivery feature switch
      xianyu_delivery_enabled: form.xianyu_delivery_enabled,
      // Model Plaza feature switches + description
...
const providerKeyOptions = computed(() => [
  ...
  { value: "xunhupay", label: t("admin.settings.payment.providerXunhu") },
