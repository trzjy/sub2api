export default {
  pricing: {
    ...
    cost: {
      ...
      measured: 'measured',
      accounts: 'Bound accounts (IDs)',
      accountsPlaceholder: 'e.g. 44, 36, 32',
      accountsHint: 'For experiments: the first account with AK/SK probes usage; the account whose model mapping contains the target model handles the burst',
      runExperiment: 'Measure',
      runExperimentHint: 'Start a controlled experiment: probe official usage → burst calls → wait for metering → re-probe to solve cost (~15 min per model)',
      experimentRunning: 'Measuring…',
      experimentStarted: 'Experiment started, ~15 min per model (metering wait included)',
