...
type Group struct {
	...
	// 订阅分组并发上限，0 表示不限制；订阅模式下每个使用者各自并发上限，与用户全局并发互不干扰
...
type GroupEdges struct {
	...
	// RedeemCodeGroups holds the value of the redeem_code_groups edge.
	RedeemCodeGroups []*RedeemCodeGroup `json:"redeem_code_groups,omitempty"`
...
// RedeemCodeGroupsOrErr returns the RedeemCodeGroups value or an error if the edge
// was not loaded in eager-loading.
func (e GroupEdges) RedeemCodeGroupsOrErr() ([]*RedeemCodeGroup, error) {
	if e.loadedTypes[2] {
		return e.RedeemCodeGroups, nil
	}
	return nil, &NotLoadedError{edge: "redeem_code_groups"}
}
...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case group.FieldID, group.FieldDefaultValidityDays, group.FieldFallbackGroupID, group.FieldFallbackGroupIDOnInvalidRequest, group.FieldSortOrder, group.FieldRpmLimit, group.FieldConcurrency:
...
// QueryRedeemCodeGroups queries the "redeem_code_groups" edge of the Group entity.
func (_m *Group) QueryRedeemCodeGroups() *RedeemCodeGroupQuery {
	return NewGroupClient(_m.config).QueryRedeemCodeGroups(_m)
}
