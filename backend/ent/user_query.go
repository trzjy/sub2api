...
func (_q *UserQuery) QueryWelfareBalances() *WelfareBalanceQuery {
...
func (_q *UserQuery) WithWelfareBalances(opts ...func(*WelfareBalanceQuery)) *UserQuery {
	query := (&WelfareBalanceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withWelfareBalances; query != nil {
		if err := _q.loadWelfareBalances(ctx, query, nodes,
			func(n *User) { n.Edges.WelfareBalances = []*WelfareBalance{} },
			func(n *User, e *WelfareBalance) { n.Edges.WelfareBalances = append(n.Edges.WelfareBalances, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadWelfareBalances(ctx context.Context, query *WelfareBalanceQuery, nodes []*User, init func(*User), assign func(*User, *WelfareBalance)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(welfarebalance.FieldUserID)
	}
	query.Where(predicate.WelfareBalance(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.WelfareBalancesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
