...
func (s *FrontendServer) serveIndexHTML(c *gin.Context) {
	...
	// Only the raw root path gets the homepage variant (footer-links injection);
	// /index.html renders NotFound in the SPA and must not contain the nav block.
	isHome := c.Request.URL.Path == "/"

	// Check cache first
	var cached *CachedHTML
	if isHome {
		cached = s.cache.GetHome()
	} else {
		cached = s.cache.Get()
	}
	if cached != nil {
	...
	// Capture the settings version before fetching settings so a concurrent
	// invalidation can be detected at commit time.
	...
	committed := s.cache.SetIfCurrent(rendered, homeRendered, settingsJSON, version)

	// Serve the variant matching the request path. Only a successful CAS commit
	// may pair cache state with the response: on a lost race the re-fetched
	// variant could belong to another request's newer render, and pairing it
	// with this request's body would attach a mismatched ETag. The current
	// render is served instead, without caching and without an ETag.
	if committed {
		if isHome {
			cached = s.cache.GetHome()
		} else {
	...
	if cached != nil {
		// The committed variant is served from the cache so the body and the
		// ETag always come from the same render (ours, or a newer entry that
		// raced in after the commit — both are self-consistent).
		content := replaceNoncePlaceholder(cached.Content, nonce)
	...
	// The current render matches the requested variant: the homepage variant
	// carries the footer-links nav block, the base variant does not.
	content := rendered
	if isHome {
...
// injectFooterLinks renders the configured footer_links into real <a> tags,
// wrapped in a hidden nav block, injected before the first </body>. It only
// parses, stable-sorts and HTML-escapes — no filtering — mirroring the Vue
// full-render semantics on the same SSOT.
func injectFooterLinks(html, settingsJSON []byte) []byte {
	var cfg struct {
		FooterLinks []struct {
	...
	// Stable ascending sort by sort_order, mirroring the frontend
	// sortedFooterLinks ordering.
	sort.SliceStable(cfg.FooterLinks, func(i, j int) bool {
		return cfg.FooterLinks[i].SortOrder < cfg.FooterLinks[j].SortOrder
	})

	var buf bytes.Buffer
	buf.WriteString(`<nav id="footer-links" hidden>`)
	for _, link := range cfg.FooterLinks {
		buf.WriteString(`<a href="`)
		buf.WriteString(htmlpkg.EscapeString(link.URL))
		buf.WriteString(`" target="_blank" rel="noopener noreferrer">`)
		buf.WriteString(htmlpkg.EscapeString(link.Name))
		buf.WriteString(`</a>`)
	}
	buf.WriteString(`</nav>`)

	bodyClose := []byte("</body>")
	return bytes.Replace(html, bodyClose, append(buf.Bytes(), bodyClose...), 1)
}
