...
func (s *SettingService) buildSystemSettingsUpdates(ctx context.Context, settings *SystemSettings) (map[string]string, error) {
	...
	updates[SettingKeySupportQRCodeURL] = settings.SupportQRCodeURL
	updates[SettingKeyDocURL] = settings.DocURL
	updates[SettingKeyHomeContent] = settings.HomeContent
	updates[SettingKeyCompactHomeEnabled] = strconv.FormatBool(settings.CompactHomeEnabled)
	updates[SettingKeyConcurrencyLimitMessage] = strings.TrimSpace(settings.ConcurrencyLimitMessage)
	...
	if settings.CodeBuddyDefaultRPM < 0 {
		settings.CodeBuddyDefaultRPM = DefaultCodeBuddyRPM
	}
	updates[SettingKeyCodeBuddyDefaultRPM] = strconv.Itoa(settings.CodeBuddyDefaultRPM)
	...
	updates[SettingKeyXianyuDeliveryEnabled] = strconv.FormatBool(settings.XianyuDeliveryEnabled)
