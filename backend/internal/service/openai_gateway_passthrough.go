...
func (s *OpenAIGatewayService) buildUpstreamRequestOpenAIPassthrough(
	...
	case AccountTypeAPIKey:
		...
		if account.UsesNativeCNResponses() && account.IsAdaptiveAPIProtocol() && !isVolcanoBaseURL(account.GetBaseURL()) {
			// 火山订阅号（platform=deepseek、base_url 指向 volces）不走 CN adaptive 覆盖：
			// 其 Responses 端点由 openAIResponsesURLForBase 按火山 base 推导 /v3/responses。
