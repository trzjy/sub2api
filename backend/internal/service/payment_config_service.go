...
func (s *PaymentConfigService) GetPaymentConfig(ctx context.Context) (*PaymentConfig, error) {
	keys := []string{
		...
		SettingEnabledPaymentTypes, SettingBalancePayDisabled, SettingFXRates, SettingRechargeFeeRate, SettingLoadBalanceStrategy,
...
func (s *PaymentConfigService) parsePaymentConfig(vals map[string]string) *PaymentConfig {
	...
	// FX_RATES：全局汇率表。空串 / 解析失败回退默认值（保证 CNY 可用）。
	if fxRaw := vals[SettingFXRates]; fxRaw != "" {
		if parsed, err := payment.ParseFXRates(fxRaw); err == nil {
			cfg.FXRates = parsed
		} else {
...
func (s *PaymentConfigService) UpdatePaymentConfig(ctx context.Context, req UpdatePaymentConfigRequest) error {
	if req.FXRates != nil {
		...
		// 反向校验：保存 FX_RATES 时，所有已启用 stripe/airwallex 实例的币种必须仍在表内，
		// 否则拒绝保存（避免存量渠道在 checkout 时遇 FX_RATE_MISSING 失败关闭）。
		if err := s.validateFxRatesForEnabledInstances(ctx, fxRates); err != nil {
			return err
		}
	}
		...
		case payment.TypeXunhupay:
			for _, supportedType := range splitTypes(inst.SupportedTypes) {
				if NormalizeVisibleMethod(supportedType) == payment.TypeWxpay {
					available[VisibleMethodSourceXunhupayWechat] = true
				}
			}
		}
	}
