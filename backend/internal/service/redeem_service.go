...
type RedeemCodeRepository interface {
	...
	ListWithFilters(ctx context.Context, params pagination.PaginationParams, codeType, status, search, poolSlug string, value *float64) ([]RedeemCode, *pagination.PaginationResult, error)
	// ListDistinctValues returns distinct redeem code face values (ascending), optionally scoped to a type.
	ListDistinctValues(ctx context.Context, codeType string) ([]float64, error)
