...
// 取代 IsWebProvider(platform)，方案 §5.5 / B4 口径）。
...
// 旧 web-zhipu 平台值的分组仍回落 web 模型目录（兼容读取期）。
func TestTaskBGatewayModels_LegacyWebZhipuGroupReturnsWebModels(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	h := newGatewayModelsHandlerForTest(
		&gatewayModelsAccountRepoStub{
			byGroup: map[int64][]service.Account{
				groupID: {
					{ID: 1, Platform: service.PlatformWebZhipu, Credentials: map[string]any{"cookie": "c"}},
				},
			},
		},
	)
	...
	c.Request = httptest.NewRequest(http.MethodGet, "/v1/models", nil)
	c.Set(string(middleware2.ContextKeyAPIKey), &service.APIKey{
		Group: &service.Group{ID: groupID, Platform: service.PlatformWebZhipu},
	})
